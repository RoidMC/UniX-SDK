...
---<br>
---| `范围`：`客户端`
---@param creature number 生物ID
---@param sound_id number 音效ID
---@param volume number 音量 （默认大小0，最小值为-48，最大值为12）
---@param duration number 持续时间（单位s，0为播放默认时长）
---@param tone number 音调 （有效范围最小值为-48，最大值为48）
function UDK_Sound.PlayOnCreature(creature, sound_id, volume, duration, tone)
    local soundID = Audio:PlayAudio(sound_id, Audio.TARGET_TYPE.Creature, creature, volume, duration)
    Audio:SetTone(soundID, tone)
    if soundID == -1 then
        Log:PrintError("[UDK:Sound] Creature播放音效失败，音效ID：" .. sound_id)
    end
end
